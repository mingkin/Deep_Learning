...
'修改部分为 _build_word_char_embeddings(self) '
...
def _get_feed_dict_from_X(X, start, end, model, char_inputs, bidirectional):
    ...
    if bidirectional:
        if not char_inputs:
            ...
            feed_dict[model.tokens_characters_reverse] = X['tokens_characters_reverse'][start:end]
...
def train(options, data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.device('/cpu:0'):
        ...
        train_perplexity = tf.get_variable(
            'train_perplexity', [], initializer=tf.constant_initializer(0.0), trainable=False)
        norm_summaries = []
...
def load_vocab(vocab_file,vocab_file1, max_word_length=None):
    if max_word_length:
        return UnicodeCharsVocabulary(vocab_file, vocab_file1, max_word_length,
                                      validate_file=True)
    else:
