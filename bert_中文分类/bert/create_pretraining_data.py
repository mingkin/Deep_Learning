...
flags.DEFINE_string("input_file", None,
                    "Input raw text file (or comma-separated list of files).指定输入文档路径")

flags.DEFINE_string(
    "output_file", None,
    "Output TF example file (or comma-separated list of files).指定输出路径")

flags.DEFINE_string("vocab_file", None,
                    "The vocabulary file that the BERT model was trained on.指定词典路径（谷歌已在预训练模型中提供）")

flags.DEFINE_bool(
    "do_lower_case", True,
    "Whether to lower case the input text. Should be True for uncased.为True则忽略大小写 "
    "models and False for cased models.")

flags.DEFINE_integer("max_seq_length", 128, "Maximum sequence length.每一条训练数据（两句话）相加后的最大长度限制")

flags.DEFINE_integer("max_predictions_per_seq", 20,
                     "Maximum number of masked LM predictions per sequence.每一条训练数据mask的最大数量")

flags.DEFINE_integer("random_seed", 12345, "Random seed for data generation.随机种子")

flags.DEFINE_integer(
    "dupe_factor", 10,
    "Number of times to duplicate the input data (with different masks).对文档多次重复随机产生训练集，随机的次数")

flags.DEFINE_float("masked_lm_prob", 0.15, "Masked LM probability."
  "一条训练数据产生mask的概率，即每条训练数据随机产生max_predictions_per_seq×masked_lm_prob数量的mask")

flags.DEFINE_float(
    "short_seq_prob", 0.1,
    "Probability of creating sequences which are shorter than the "
    "maximum length."
     "为了缩小预训练和微调过程的差距，以此概率产生小于max_seq_length的训练数据")
...
def create_training_instances(input_files, tokenizer, max_seq_length,
    ...
    with tf.gfile.GFile(input_file, "r") as reader:
      while True:
        ...
        if not line:
          ...
          在文件中，如果遇到一个空白行，readline()并不会返回一个空串，因为每一行的末尾还有一个或多个分隔符，
          因此“空白行”至少会有一个换行符或者系统使用的其他符号。只有当真的读到文件末尾时，才会读到空串""。
          '''
          break
        line = line.strip()
  ...
  # Remove empty documents if x 只要x是非零数值、非空字符串、非空list等，就判断为True，否则为False
...
def main(_):
  ...
  '''
  简要介绍一下FullTokenizer这个类，它以vocab_file为词典，将词转化为该词对应的id，对于某些特殊词，如johanson，
  会先将johanson按照最大长度拆分，再看拆分的部分是否在vocab_file里。vocab_file里有没有"johanson"这个词，
  但有"johan"和"##son"这两个词，所以将"johanson"这个词拆分成两个词（##表示非开头匹配）
  '''

  input_files = []
  for input_pattern in FLAGS.input_file.split(","):
    #tf.logging.info(input_pattern)
