    ...
    def _blstm_atten(self):
        ...
        with tf.variable_scope("output"):
            output_w = tf.get_variable(
                "output_w", shape=[output_size, self.num_label],
                initializer=tf.contrib.layers.xavier_initializer())
            output_b = tf.get_variable("output_b", initializer=tf.constant(0.1, shape=[self.num_label]))
