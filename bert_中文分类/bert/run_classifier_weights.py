...
flags.DEFINE_string("task_name", "ming", "The name of the task to train. defalut ming(self_define)")
...
class DataProcessor(object):
  ...
  def _read_tsv(cls, input_file, quotechar=None):
    """Reads a tab separated value file.读文件分隔符"""
    '''格式['ad03', '1', '', 'It is easy to slay the Gorgon.']'''
...
class MingProcessor(DataProcessor):
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    if FLAGS.weights:
        # class weight
        tf.logging.info("create weight cross entropy pos_weight=" + FLAGS.weights)
        pos_weight = tf.constant([float(p) for p in FLAGS.weights.split(",")])
